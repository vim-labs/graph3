...
contract Graph3 {
    ...
    // 1: {}
    event Prop(string indexed _id, string indexed _prop, string indexed _value);

    // 2: ->
    event Link(string indexed _from, string indexed _to, string indexed _class);

    event Batch(uint256[] _events, string[3][] _args, bytes32[] _sig);

    // Create a node or change class (drop with empty class)
    function node(
        string memory _id,
        string memory _class,
        string memory _subclass
    ) public {
        emit Node(_id, _class, _subclass);
    }

    // Add a property to a node or a link (drop with empty prop/value)
    function prop(string memory _id, string memory _prop, string memory _value)
        public
    {
    ...
    function link(string memory _from, string memory _to, string memory _class)
        public
    {
